...
class schedule extends SIAP_Controller {
	...
	function getListSchedule(){
		...
		$params = array(
			'v_id' => ifunsetempty($_POST,'satkerid',null),
		...
		$mresult = $this->m_schedule->getListSchedule($params);
	...
	function proses_import_file(){
		...
		foreach($params as $key => $val) {
			...
			if($key !== 'No.') {
				...
				$mresultSchedule = $this->m_schedule->getSchedule($sendparams);

	            if(count($mresultSchedule) > 0) {
	            	// update when have a same value
	            	$mresult = $this->m_schedule->updateSchedule($temp);
					if($mresult) $o++;
				} else {
					// insert when have a new value
					$mresult = $this->m_schedule->addPengajuanSchedule($temp);
					if($mresult) $o++;
				}
			}
		}		
