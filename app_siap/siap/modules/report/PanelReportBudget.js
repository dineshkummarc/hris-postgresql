Ext.define("SIAP.modules.report.PanelReportBudget", {
	extend: "Ext.grid.Panel",
	alternateClassName: "SIAP.panelreportbudget",
	alias: 'widget.panelreportbudget',
	requires: [
	...
	initComponent: function(){
		...
		var storebudget = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: Settings.SITE_URL + '/report/getReportBudget',
				actionMethods: {
					create : 'POST',
					read   : 'POST',
				},								
			...
			fields: [
				'id','pengajuanid','pegawaiid','jenisform','nik','namadepan','nourut','grandtotal','idaccomodation','satkerid','tglmulai','tglselesai','kota'
			],
			listeners: {
		...
		Ext.apply(me, {
			title: 'Advance', xtype: 'grid', region: 'center', layout: 'fit', autoScroll: true, frame: false, border: true, loadMask: true, stripeRows: true,
			...
			columns: [
				...
				//{header: 'Tgl Terima Dept', dataIndex: 'tglheadga', width: 90},	
			...
			/*{glyph:'xf02f@FontAwesome', text: 'Cetak',
					...
					Ext.getStore('storebudget').proxy.extraParams.satkerid = treeid;
					var m = Ext.getStore('storebudget').proxy.extraParams;
