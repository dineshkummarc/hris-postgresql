Ext.define('SIAP.modules.perjalanandinas.GridCuti', {
    extend: 'Ext.grid.Panel',
    alternateClassName: 'SIAP.gridcuti',
    alias: 'widget.gridcuti',
    requires: ['SIAP.components.field.ComboStatusPjdinas', 'SIAP.components.field.ComboJenisFormPjdinas'],
    initComponent: function () {
        ...
        var storecuti = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                url: Settings.SITE_URL + '/perjalanandinas/getListCuti',
                actionMethods: {
                    create: 'POST',
                    read: 'POST',
                },
        ...
        Ext.apply(me, {
            ...
            columns: [
                    ...
                    renderer: function (value, p, record) {
                        if (Settings.userid == '1235' || Settings.userid == '108' || Settings.userid == '244') {
                            ...
                            if (Settings.userid == '1235') {
                                if (record.data['tgladmga'] != null) {
                                    return record.data['tgladmga'];
                                } else {
                                    return '-';
                                }
                            } else if (Settings.userid == '108') {
                                if (record.data['tglassmanga'] != null) {
                                    return record.data['tglassmanga'];
                                } else {
                                    return '-';
                                }
                            } else {
                                if (record.data['tglheadga'] != null) {
                                    return record.data['tglheadga'];
                                } else {
                                    return '-';
                                }
                            }
                        } else if (Settings.userid == '139' || Settings.userid == '93') {
                            //FA departemen
                            if (Settings.userid == '139') {
                                if (record.data['tgladmfa'] != null) {
                                    return record.data['tgladmfa'];
                                } else {
                                    return '-';
                                }
                            } else {
                                if (record.data['tglheadfa'] != null) {
                                    return record.data['tglheadfa'];
                                } else {
                                    return '-';
                                }
                            }
                ...
                { header: 'Jenis Form', dataIndex: 'jenisform', width: 120 },
                {
                    header: 'Status',
                    dataIndex: 'status',
                    width: 120,
                    renderer: function (value, meta, record, index) {
                        if (record.data.statusid == '4' || record.data.statusid == '6' || record.data.statusid == '8' || record.data.statusid == '10') {
                ...
                { header: 'Approval', dataIndex: 'atasan2nama', width: 120 },
                {
                    ...
                    renderer: function (value, meta, record, index) {
                        return (
                            ...
                            ',' +
                            record.data.jenisformid +
                            ',' +
                            record.data.jenistiketid +
                            ',' +
                            record.data.id +
                            ',' +
                            record.data.sharingbudget +
...
function detail(pegawaiid, nourut, periode, statusid, jenisformid, jenistiketid, pengajuanid, sharingbudget) {
    Ext.History.add(
        '#perjalanandinas&detailperjalanan&' +
            ...
            '#' +
            Base64.encode(jenisformid) +
            '#' +
            Base64.encode(jenistiketid) +
            '#' +
            Base64.encode(pengajuanid) +
            '#' +
            Base64.encode(sharingbudget)
    );
}
        ...
        items: [
            {
                ...
                items: [{ xtype: 'displayfield', value: record.get('verifikasinotes') }],
                buttons: [
                    {
                        text: 'Cancel',
                        handler: function () {
