Ext.define("SIAP.modules.master.MasterJabatan", {
	extend: "Ext.panel.Panel",
	alternateClassName: "SIAP.masterjabatan",
	alias: 'widget.masterjabatan',	
	requires: [
		'SIAP.components.tree.Jabatan',
	],		
	initComponent: function(){
		...
		Ext.apply(me,{		
			...
			items: [
				Ext.create('SIAP.components.tree.Jabatan', {
					...
					handler: function(){						
						if(win.down('form').getForm().isValid()){
							win.down('form').getForm().submit({
								url: Settings.MASTER_URL + '/c_jabatan/CRUDMasterJabatan',
								waitTitle:'Menyimpan...', 
	...
	hapus: function(record){
		...
		Ext.Msg.show({
			...
			fn: function(btn){
				if(btn == 'yes'){
					Ext.Ajax.request({
						url: Settings.MASTER_URL + '/c_jabatan/hapus',
						method: 'POST',
						params: {
