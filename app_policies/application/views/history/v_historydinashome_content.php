	...
	<div class="row" style="padding-left:5px;padding-right:5px;">
		<div class="pull-left">
			...
			<div class="div_filter">
				<div class="column1">
					<select id="id_dokumenid" class="form-control" name="dokumenid">
...
<script type="text/javascript">
	$(function() {
		...
		$.fn.load_grid_content = function(keyword, newsatker, lokasikerja, dokumenid, start, limit) {
			$.getJSON(
				SITE_URL + '/history/getListPolicies', 'keyword=' + keyword + '&newsatker=' + newsatker + '&lokasikerja=' + lokasikerja + '&dokumenid=' + dokumenid + '&start=' + start + '&limit=' + limit,
				function(response) {
					var html = '';
		...
		$.fn.lihatdokumen = function(files) {
			window.location = SITE_URL + '/history/detaildokumen?files=' + Base64.encode(files);
		};

		$.fn.edit = function(id) {
			window.location = SITE_URL + '/history/edit?id=' + Base64.encode(id);
		};
		...
		var dokumenid = $('#id_dokumenid').val();
		...
		$.fn.load_grid_content(keyword, newsatker, lokasikerja, dokumenid, start, limit);
		...
		$('#id_search').on('click', function(e) {
			...
			$.fn.load_grid_content(keyword, jenisid, lokasikerja, dokumenid, start, limit);
		});
	});
</script>